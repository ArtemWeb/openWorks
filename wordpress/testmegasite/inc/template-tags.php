...
function get_attachment_alt( int $attachment_id ): string {
	...
	if ( ! empty( $img_alt ) ) {
		...
		return esc_html__( 'image description', 'base' );
	}
}
...
function theme_the_image_2x($img_id, $img_lg_id) {
	$image_url = wp_get_attachment_url($img_id);
	$image_lg_url = wp_get_attachment_url($img_lg_id);
	
	if ($image_url && $image_lg_url) {
			echo '<img src="' . esc_url($image_url) . '" srcset="' . esc_url($image_url) . ' 1x, ' . esc_url($image_lg_url) . ' 2x" alt="' . esc_attr(get_post_meta($img_id, '_wp_attachment_image_alt', true)) . '" />';
	} elseif ($image_url) {
			echo '<img src="' . esc_url($image_url) . '" alt="' . esc_attr(get_post_meta($img_id, '_wp_attachment_image_alt', true)) . '" />';
	}
}